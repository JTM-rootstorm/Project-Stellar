...
Replace the active branch direction from glTF-authored level work to BSP-canonical level work before implementation agents modify core code.

This phase prevents Kilo agents from continuing obsolete collision/glTF assumptions.
...
- Preserve assumptions:
  ...
  - no glTF functionality.
- State that older glTF/collision phase plans are historical only.
...
- State that glTF is to be removed, not kept side-by-side.
...
Do not delete useful completion history. Mark old glTF/collision authoring notes as historical, not active.
...
- Remove statements claiming glTF is the runtime scene/level source.
- Add a temporary migration note: active implementation is moving from glTF-shaped `SceneAsset` toward BSP-backed `LevelAsset`.
- Retain server-authoritative scripting and collision rules.

### 4. `AGENTS.md`

Because the user explicitly changed the project direction, update only coordination/routing references needed to avoid agents continuing glTF work.

Allowed edits:

- Replace glTF level-import routing references with BSP level-import routing references.
...
- “glTF rendering/import” → “BSP level geometry import/rendering where applicable.”
- “glTF collision extraction” → “BSP level collision/entity metadata extraction.”
- “Current renderer goal is lightweight material parity, not full glTF PBR” → “Current renderer goal is lightweight BSP surface/material and billboard parity, not full PBR.”
...
- `docs/Design.md` no longer says glTF is the canonical playable world/level source.
- `AGENTS.md` no longer routes new level work through glTF terminology.
...
```bash
rg -n -i "gltf|cgltf|STELLAR_ENABLE_GLTF" docs AGENTS.md Plans/NEXT.md
```

Expected:

- Active docs should not describe glTF as a current or future required level pipeline.
