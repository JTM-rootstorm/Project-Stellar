...
std::size_t count_collision_triangles(
...
std::size_t
count_marker_type(const stellar::assets::WorldMetadataAsset &metadata,
                  stellar::assets::WorldMarkerType type) noexcept {
  std::size_t total = 0;
  for (const auto &marker : metadata.markers) {
    if (marker.type == type) {
...
RuntimeWorld build_runtime_world(const stellar::assets::LevelAsset &level) {
  ...
  if (level.level_collision.has_value()) {
    world.diagnostics.collision_mesh_count =
        level.level_collision->meshes.size();
    world.diagnostics.collision_triangle_count =
        count_collision_triangles(*level.level_collision);
  ...
  world.diagnostics.sprite_marker_count = count_marker_type(
      world.world_metadata, stellar::assets::WorldMarkerType::kSprite);
  world.diagnostics.object_collider_marker_count = count_marker_type(
      world.world_metadata, stellar::assets::WorldMarkerType::kObjectCollider);
  world.diagnostics.has_player_spawn = has_marker_type(
      world.world_metadata, stellar::assets::WorldMarkerType::kPlayerSpawn);
...
const stellar::assets::WorldMarker *
...
std::vector<const stellar::assets::WorldMarker *>
