...
constexpr GLenum
texture_format_from_image(stellar::assets::ImageFormat format,
  ...
  case stellar::assets::ImageFormat::kR8G8B8:
    internal_format = color_space == stellar::assets::TextureColorSpace::kSrgb
                          ? GL_SRGB8
                          : GL_RGB8;
...
GLint to_gl_filter(stellar::assets::TextureFilter filter,
                   GLint fallback) noexcept {
  switch (filter) {
...
void apply_sampler_state(
    const stellar::assets::SamplerAsset &sampler) noexcept {
...
OpenGLGraphicsDevice::initialize(stellar::platform::Window &window) {
  ...
  if (!window_) {
    return std::unexpected(
        stellar::platform::Error("Window is not initialized"));
  }
  ...
  auto shader_program =
      create_shader_program_impl(kVertexShader, kFragmentShader);
  if (!shader_program) {
    SDL_GL_DeleteContext(context_);
    context_ = nullptr;
...
OpenGLGraphicsDevice::create_mesh(const stellar::assets::MeshAsset &mesh) {
  if (!context_) {
    return std::unexpected(
        stellar::platform::Error("Graphics device is not initialized"));
  }
  ...
  for (const auto &primitive : mesh.primitives) {
    if (primitive.topology != stellar::assets::PrimitiveTopology::kTriangles) {
      return std::unexpected(
          stellar::platform::Error("Unsupported mesh topology"));
    }
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE,
                          ...
                          reinterpret_cast<void *>(offsetof(
                              stellar::assets::StaticVertex, position)));
    ...
    glVertexAttribPointer(
        2, 2, GL_FLOAT, GL_FALSE, sizeof(stellar::assets::StaticVertex),
        reinterpret_cast<void *>(offsetof(stellar::assets::StaticVertex, uv0)));
    glEnableVertexAttribArray(2);

    glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE,
                          sizeof(stellar::assets::StaticVertex),
                          reinterpret_cast<void *>(offsetof(
                              stellar::assets::StaticVertex, tangent)));
    glEnableVertexAttribArray(3);

    glVertexAttribPointer(
        4, 2, GL_FLOAT, GL_FALSE, sizeof(stellar::assets::StaticVertex),
        reinterpret_cast<void *>(offsetof(stellar::assets::StaticVertex, uv1)));
...
OpenGLGraphicsDevice::create_texture(const TextureUpload &texture) {
  ...
  if (texture_format_from_image(image.format, texture.color_space,
                                internal_format, external_format) == GL_NONE) {
    return std::unexpected(
        stellar::platform::Error("Unsupported image format"));
  }
...
void OpenGLGraphicsDevice::draw_mesh(
    MeshHandle mesh, std::span<const MeshPrimitiveDrawCommand> commands,
  ...
  for (const MeshPrimitiveDrawCommand &command : commands) {
    ...
    const MaterialRecord *material =
        material_it != materials_.end() ? &material_it->second : nullptr;
    const std::array<float, 4> fallback_color{0.85f, 0.9f, 1.0f, 1.0f};
    const std::array<float, 4> base_color =
        material != nullptr ? material->upload.material.base_color_factor
                            : fallback_color;

    const auto alpha_mode = material != nullptr
                                ? material->upload.material.alpha_mode
                                : stellar::assets::AlphaMode::kOpaque;
    if (alpha_mode == stellar::assets::AlphaMode::kBlend) {
      // RenderLevel orders transparent primitives; direct device submissions
      // should already be preordered by the caller.
      glEnable(GL_BLEND);
    ...
    if (material != nullptr &&
        material->upload.base_color_texture.has_value()) {
    ...
    if (material != nullptr && primitive.has_tangents) {
      has_normal_texture =
          bind_texture(material->upload.normal_texture, GL_TEXTURE1);
    }

    bool has_metallic_roughness_texture = false;
    if (material != nullptr) {
      has_metallic_roughness_texture = bind_texture(
          material->upload.metallic_roughness_texture, GL_TEXTURE2);
    }

    bool has_occlusion_texture = false;
    if (material != nullptr) {
      has_occlusion_texture =
          bind_texture(material->upload.occlusion_texture, GL_TEXTURE3);
    }
    ...
    const GLint occlusion_texture_loc =
    ...
    const GLint has_mr_loc = glGetUniformLocation(
        shader_program_, "u_has_metallic_roughness_texture");
    const GLint has_occlusion_loc =
    ...
    const GLint mr_texcoord_loc = glGetUniformLocation(
        shader_program_, "u_metallic_roughness_texcoord_set");
    const GLint occlusion_texcoord_loc =
    ...
    const GLint alpha_cutoff_loc =
        glGetUniformLocation(shader_program_, "u_alpha_cutoff");
    const GLint unlit_loc = glGetUniformLocation(shader_program_, "u_unlit");
    const GLint texture_transform0_loc =
        glGetUniformLocation(shader_program_, "u_texture_transform0[0]");
    const GLint texture_transform1_loc =
    ...
    glUniform1f(metallic_loc, material != nullptr
                                  ...
                                  : 0.0f);
    glUniform1f(roughness_loc, material != nullptr
    ...
    const std::array<std::array<float, 4>, 5> transform0{
        ...
        material != nullptr ? transform0_for(material->upload.normal_texture)
                            : std::array<float, 4>{0.0F, 0.0F, 0.0F, 0.0F},
        material != nullptr
            ? transform0_for(material->upload.metallic_roughness_texture)
            : std::array<float, 4>{0.0F, 0.0F, 0.0F, 0.0F},
    ...
    const std::array<std::array<float, 4>, 5> transform1{
        ...
        material != nullptr ? transform1_for(material->upload.normal_texture)
                            : std::array<float, 4>{1.0F, 1.0F, 0.0F, 0.0F},
        material != nullptr
            ? transform1_for(material->upload.metallic_roughness_texture)
            : std::array<float, 4>{1.0F, 1.0F, 0.0F, 0.0F},
    ...
    glUniform4fv(texture_transform1_loc,
                 static_cast<GLsizei>(transform1.size()), transform1[0].data());
...
void OpenGLGraphicsDevice::destroy_texture_record(
    TextureRecord &record) noexcept {
  if (record.texture != 0) {
