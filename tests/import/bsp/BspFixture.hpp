...
inline void append_two_face_lumps(std::vector<std::byte> &bytes) {
    const std::string entities = "{\n\"classname\" \"worldspawn\"\n}\n";
    const std::size_t entity_offset = bytes.size();
    bytes.insert(bytes.end(), reinterpret_cast<const std::byte *>(entities.data()),
                 reinterpret_cast<const std::byte *>(entities.data() + entities.size()));
    ...
    append_vec3(bytes, 0.0F, 16.0F, 0.0F);
    append_vec3(bytes, 16.0F, 0.0F, 0.0F);
    append_vec3(bytes, 32.0F, 0.0F, 0.0F);
    append_vec3(bytes, 32.0F, 16.0F, 0.0F);
    append_vec3(bytes, 16.0F, 16.0F, 0.0F);
    ...
    const std::size_t face_offset = bytes.size();
    for (std::int32_t light_offset : {0, 12}) {
        append<std::uint16_t>(bytes, 0);
        append<std::uint16_t>(bytes, 0);
        append<std::int32_t>(bytes, light_offset == 0 ? 0 : 4);
        append<std::uint16_t>(bytes, 4);
        append<std::uint16_t>(bytes, 0);
        append<std::uint8_t>(bytes, light_offset == 0 ? 0 : 2);
        append<std::uint8_t>(bytes, 255);
        append<std::uint8_t>(bytes, 255);
        append<std::uint8_t>(bytes, 255);
        append<std::int32_t>(bytes, light_offset);
    }
    set_lump(bytes, LumpIndex::kFaces, face_offset, 2 * 20);

    const std::size_t edge_offset = bytes.size();
    for (std::uint16_t base : {0, 4}) {
        append<std::uint16_t>(bytes, base + 0);
        append<std::uint16_t>(bytes, base + 1);
        append<std::uint16_t>(bytes, base + 1);
        append<std::uint16_t>(bytes, base + 2);
        append<std::uint16_t>(bytes, base + 2);
        append<std::uint16_t>(bytes, base + 3);
        append<std::uint16_t>(bytes, base + 3);
        append<std::uint16_t>(bytes, base + 0);
    }
    set_lump(bytes, LumpIndex::kEdges, edge_offset, 32);

    const std::size_t surfedge_offset = bytes.size();
    for (std::int32_t edge = 0; edge < 8; ++edge) {
        append<std::int32_t>(bytes, edge);
    }
    set_lump(bytes, LumpIndex::kSurfedges, surfedge_offset, 32);
...
inline std::vector<std::byte> two_lit_faces_bsp(std::string texture_name = "stone") {
    std::vector<std::byte> bytes(4 + 15 * 8);
    patch_i32(bytes, 0, 29);
    ...
    append_two_face_lumps(bytes);

    const std::size_t lighting_offset = bytes.size();
    for (std::size_t i = 0; i < 24; ++i) {
        append<std::uint8_t>(bytes, static_cast<std::uint8_t>((i * 7U) & 0xFFU));
    }
    set_lump(bytes, LumpIndex::kLighting, lighting_offset, 24);
    return bytes;
}
