...
struct RuntimeWorldDiagnostics {
  ...
  /** @brief Number of source collision triangles available to the runtime
   * world. */
  ...
  /** @brief Number of imported object collider markers exposed for gameplay
   * sensors. */
...
/**
 * @brief Backend-neutral runtime world assembled from a source-neutral level
 * asset.
 *
 * RuntimeWorld does not own the source LevelAsset. The caller must keep the
 * LevelAsset alive for at least as long as this object because CollisionWorld
 * references LevelAsset::level_collision data.
 */
struct RuntimeWorld {
  /** @brief Source level asset backing geometry metadata and collision
   * lifetime. */
  const stellar::assets::LevelAsset *level_asset = nullptr;

  /** @brief Optional static collision query world, created only for non-empty
   * level collision. */
...
/**
 * @brief Assemble a backend-neutral runtime world from a source-neutral level
 * asset.
 */
[[nodiscard]] RuntimeWorld
build_runtime_world(const stellar::assets::LevelAsset &level);

/**
 * @brief Find the first player spawn marker in runtime world metadata.
 */
[[nodiscard]] const stellar::assets::WorldMarker *
find_player_spawn(const RuntimeWorld &world) noexcept;

/**
 * @brief Find all markers of a metadata type in runtime world metadata.
 */
[[nodiscard]] std::vector<const stellar::assets::WorldMarker *>
find_markers_by_type(const RuntimeWorld &world,
                     stellar::assets::WorldMarkerType type);

/**
 * @brief Find the first entity spawn marker whose archetype matches the
 * supplied name.
 */
[[nodiscard]] const stellar::assets::WorldMarker *
find_spawn_by_archetype(const RuntimeWorld &world,
 ...
 */
[[nodiscard]] std::vector<const stellar::assets::WorldMarker *>
find_sprite_markers(const RuntimeWorld &world);

/**
 * @brief Find all trigger markers exposed for later gameplay hook binding.
 */
[[nodiscard]] std::vector<const stellar::assets::WorldMarker *>
find_trigger_markers(const RuntimeWorld &world);

/**
 * @brief Find all object-collider markers exposed for runtime sensor
 * construction.
 */
[[nodiscard]] std::vector<const stellar::assets::WorldMarker *>
find_object_collider_markers(const RuntimeWorld &world);
